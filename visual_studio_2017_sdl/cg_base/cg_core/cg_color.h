...
static inline cg_color_t cg_color_mix(cg_color_t foreground, cg_color_t background, cg_alpha_t mix)
{
    cg_color_t ret;
#if CG_COLOR_DEPTH != 1
    ret.red = ((uint16_t)foreground.red *(255 - mix) + (background.red * mix)) >> 8;
    ret.green = ((uint16_t)foreground.green *(255 - mix) + (background.green * mix)) >> 8;
    ret.blue = ((uint16_t)foreground.blue *(255 - mix) + (background.blue * mix)) >> 8;
